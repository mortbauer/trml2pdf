...
class ShrinkFrame(doctemplate.FrameActionFlowable):
    ...
    def frameAction(self, frame):
        print('shrink to',frame.id,frame._y)
        frame._y1 = frame._y
        frame._y1p = frame._y1 + frame._bottomPadding 
        print(frame._y1p,frame._y1)
...
class Frame(frames.Frame):
    ...
    def init(self,lastframe):
        ...
        self._geom()
        self._reset()
        print('frame init',self._flex,self._y1,self._height)
...
class MultiColumns(doctemplate.ActionFlowable):
    ...
    def apply(self, doc_tmpl):
        ...
        if self.ref is not None:
            print('aaaaaaaaaaaaaaaaaaaa',self.ref.available_width)
